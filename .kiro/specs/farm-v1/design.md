...
输入错误边界：文件上传为空、打印机录入 IP 缺失、打印机更新 ID 缺失、文件夹名称为空以及状态历史查询体缺失均使用业务码 `400`。Controller 的参数检查与 Service 的防御性检查保持一致，便于前端获得稳定的参数错误提示；持久化失败、设备离线和任务状态不允许等异常继续使用各自的业务码。

任务派发边界：调度派发、手动安全派发和兼容派发必须共用“打印机为 `IDLE` 且 `currentJobId=null`”的可用性条件。若设备状态与绑定字段不一致，手动接口返回 HTTP/业务码 `409`，调度器跳过本次派发；任何路径都不得覆盖已有任务绑定。
...
数据库状态更新必须发生在设备调用成功之后；任务创建也必须检查 `save` 的实际结果，插入失败立即抛出业务异常，不返回任务 ID。成功事件在事务成功后发布。自动派发由 `PrintJobService.assignQueuedJob` 执行，调度器本身只负责分布式锁和扫描；该 Service 事务内重新校验 `QUEUED`/`IDLE`，依次保存任务和打印机，任一保存失败抛出业务异常并回滚，之后才发布任务事件。
