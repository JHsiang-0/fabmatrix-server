...
- [x] T2.4 验证真实 RRF 空密码设备并修正客户端边界。
  ...
  - 验收：真实目标 `192.168.0.77` 的 `rr_connect`、`state/job` 只读探测证据已登记；客户端不再无条件拒绝空密码，并兼容该设备 `err=0` 但不返回 `sessionKey` 的响应；带密码、空密码和无 sessionKey 路径均有自动化测试；控制/上传/启动响应和无动作探针任务已完成该设备级补充验证。
...
- [ ] T9.6 完成上传文件到打印完成的端到端测试。
  ...
  - 现场验证（2026-09-03）：在真实目标 `.77` 上传安全等待文件（文件 ID `46`，20 秒 `G4`，无运动/加热/挤出），创建任务 7，完成派发、安全确认和启动；RRF `state.status=idle`，`job.timesLeft` 为空且文件位置已处理，随后将 Farm 任务 7 精确收尾为 `COMPLETED`、进度 100% 并解绑设备。该结果证明设备端完成和业务数据收尾路径可执行，但不是监控任务自然更新证据。
  - 阻塞与配合：开发环境 `farm.tasks.enabled=false`，尚未在监控任务开启时验证自然完成和 WebSocket 状态变化；临时开启监控会同时启用自动调度，可能派发其他历史队列任务，因此本次未继续保持开启。要完成 T9.6，需要在隔离的测试数据库/队列或增加设备白名单后重测，并由用户现场观察真实 G-code 的运动、加热和完成状态。
...
- [x] T10.2 ADMIN 可添加 Klipper 或 RRF 打印机。
  - 验收：管理员已将真实目标 `192.168.0.77` 登记为 Farm 打印机 ID `564`，数据库协议类型为 `RRF`、空密码；根据 RRF 只读状态 `idle` 同步为 Farm `IDLE`。此前 `192.168.0.62` 的 ID `563` 仅为误输入产生的历史测试记录，不作为真实设备验收目标。
- [x] T10.3 ADMIN/OPERATOR 可查看设备状态和文件库。
  - 验收：管理员真实请求已读取打印机分页、详情和文件预览；`SecurityResponseTest` 覆盖核心查询接口的认证边界；真实目标 RRF 设备登记后，2026-09-03 本地 dev 后端返回 48 台打印机和 2 个文件（包含本次探针文件）。
...
- [~] T10.5 支持暂停、恢复、取消和急停。
  ...
  - 现场验证（2026-09-03）：使用任务 6 和安全等待文件（文件 ID `45`）在 `.77` 实际进入 `processing`；暂停后 RRF 延迟读取为 `paused`，Farm 恢复路由返回 200 且设备恢复 `processing`。取消路由返回 200，Farm 任务变为 `CANCELLED` 并解绑；RRF 对该长等待命令仍短暂报告 `processing`，最终通过 `.77` 的 `M112`/`M999` 完成急停和复位，恢复 `idle`。Farm 急停路由和 RRF 原生命令均已返回成功。
  - 阻塞与配合：由于监控关闭，暂停后的 Farm 状态需人工同步才能调用恢复，说明真实监控关闭时无法完成完整状态链路；取消后 RRF 的终态也存在延迟/协议表现差异。仍需在隔离队列、监控开启的条件下，由用户现场确认真实运行任务的暂停、恢复、取消和急停状态；当前测试文件不包含运动、加热或挤出。
