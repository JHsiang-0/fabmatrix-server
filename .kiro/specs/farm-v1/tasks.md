...
- [~] T10.6 REST 与 WebSocket 状态正确同步。
  - 当前证据：2026-09-03 使用同一管理员会话读取 REST 打印机分页 `total=46`，随后连接 `/ws/farm-status` 收到 `SNAPSHOT.data.printers` 46 条，数量一致且首条数据未包含 `apiKey`；前端已修复 `data.printers` 快照解析。仍待浏览器端持续增量事件验收。
