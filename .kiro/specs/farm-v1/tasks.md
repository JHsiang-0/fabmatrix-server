...
- [x] T10.3 ADMIN/OPERATOR 可查看设备状态和文件库。
  - 验收：管理员真实请求已读取打印机分页、详情和文件预览；`SecurityResponseTest` 覆盖核心查询接口的认证边界；2026-09-03 本地 dev 后端返回 46 台打印机和 1 个文件。
- [ ] T10.4 用户可上传文件、创建任务、派发、安全确认并启动。
- [ ] T10.5 支持暂停、恢复、取消和急停。
- [x] T10.6 REST 与 WebSocket 状态正确同步。
  - 验收：2026-09-03 使用同一管理员会话读取 REST 打印机分页 `total=46`，随后通过真实前端 Vite 代理连接 `/ws/farm-status`，收到 `SNAPSHOT.data.printers` 46 条，数量一致且首条数据未包含 `apiKey`；前端已修复 `data.printers` 快照解析并由测试覆盖。由于开发环境关闭监控任务且未操作真实设备，本次不宣称自然状态增量和完整打印链路已验收。
...
- [x] T10.10 Klipper/RRF 均通过适配器接入，核心链路有自动化测试。
  - 验收：`PrinterProtocolAdapterFactory` 统一选择 Klipper/RRF；适配器测试覆盖状态映射、暂停/恢复/取消/急停、上传及 RRF HTTP 会话，后端全量测试此前 214 项通过。真实 RRF 控制副作用仍保留现场验收。
