...
- [~] T1.3 已定义批量上传结果、预览/确认请求与逐项计划结果、计划版本/确认令牌摘要和数量上限；文件上传幂等键与计划资源版本仍待 T4 完成。
...
- [~] T9.5 已保存计划版本、过期时间和确认令牌摘要；资源版本快照和过期/篡改/资源变化完整测试待 T4/T10.6 补齐。
...
- [~] T10.6 已覆盖预览无副作用和重复确认基础测试；批量并发、部分失败、过期计划和 WebSocket/REST 一致性测试待补齐。
