...
- [~] T5.5 RRF/Klipper 协议适配器单元测试已通过，并已在 `.77` 完成响应级上传、启动、暂停、取消、急停探测；设备物理运动/加热/挤出和完整打印结果仍需现场观察。
...
- [~] T7.2 已由服务层、协议适配器和 Controller 测试覆盖暂停/恢复、取消、急停及设备不可达错误分类和状态回写；真实设备最终状态与物理副作用仍需 `.77` 现场观察。
- [~] T7.3 已在 `.77` 执行无运动/无加热/无挤出的安全探针和控制响应验证；真实打印过程中的物理动作仍需用户现场观察。
- [~] T7.4 已记录任务 ID、设备原始状态、Farm 状态和响应结果到 `API_HANDOFF.md`；自然监控增量、浏览器端实时状态和人工物理观察仍待补录。
...
- [~] T14.3 已在 `.77` 完成只读状态、无动作探针、单任务安全上传/启动/暂停/取消/急停的 HTTP 响应级验收；物理动作、监控自然同步和完整打印链路仍待观察。
- [x] T14.4 已核对 `.62` 不在控制白名单，`farm.monitor.enabled`/`farm.scheduler.enabled` 默认关闭，旧 `farm.tasks.enabled` 不会误启动新任务；配置测试通过。
- [~] T14.5 已保存请求摘要、响应和代码/设备探测结论到 `API_HANDOFF.md`；待用户现场观察后补充物理结果和最终签收结论。
