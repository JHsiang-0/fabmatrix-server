...
- [~] T9.5 已保存计划版本、过期时间、确认令牌摘要和资源版本摘要，并覆盖过期/资源变化测试；令牌篡改测试仍待补齐。
...
- [~] T13.2 已有单任务队列/文件基础页面；安全确认后启动的完整页面串联仍待完成。
- [x] T13.3 已完成批量上传、文件/设备选择、匹配预览、冲突提示、确认和逐项结果页面。
- [x] T13.4 页面只提供用户主动触发的匹配，不展示后台自动派单入口或开关。
- [~] T13.5 已接入 `sequence` 断档检测、REST 打印机快照恢复和离线状态；上传失败重试、空/维护态和浏览器真实端到端仍待补齐。
...
- [~] T16.4 已创建数据目录、阻断本地路径穿越、检查可写和最低磁盘空间，并提供 SQLite+文件备份脚本；正式恢复演练和迁移检查仍待完成。
...
- [~] T17.5 已完成 Dockerfile/Compose 静态校验和开发探活记录；正式镜像构建、生产配置安全检查和发布说明仍需收口。
