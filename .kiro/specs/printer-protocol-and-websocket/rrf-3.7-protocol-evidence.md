...
状态：已完成官方资料核对、可复现 HTTP 协议测试、一台目标 RRF 设备的只读探测，以及该设备的控制/上传/启动响应验证；由于设备报告 `isEmulated=true` 且始终处于 `off`、无任务，真实物理打印完成链路仍未验收。
...
- 初始只读探测阶段没有发送 `rr_gcode`、`rr_upload`、`M0`、`M112` 或启动命令，因此当时不构成控制/上传/打印完成验收。设备响应中的 `isEmulated=true` 和 `boardType=unknown` 仍需要按真实硬件或兼容网关进一步确认。
- 2026-09-03 在用户明确限定的唯一目标 `192.168.0.62` 上补充验证：`M25`、`M24`、`M0`、`M112` 均通过 `GET /rr_gcode` 返回 `err=0`；上传无运动/无加热探针文件 `farm_t10_rrf_probe.gcode` 到 `0:/gcodes/` 返回 `err=0`；发送 `M32 "0:/gcodes/farm_t10_rrf_probe.gcode"` 返回 `err=0`，随后发送 `M0` 收尾。Farm 后端设备 ID `563` 的暂停和急停接口分别返回 HTTP 200；恢复和取消因无绑定任务返回 HTTP 422，未绕过安全校验。
- 上述命令的响应只能证明目标 HTTP 接口接受请求；设备每次仍返回 `isEmulated=true`、`state.status=off`、空任务，因此不能证明真实运动、加热、暂停恢复或打印完成已发生。未向其他 IP 发送任何 RRF 控制、上传或启动请求。
