...
状态：已完成官方资料核对、可复现 HTTP 协议测试、真实目标 RRF 设备的只读探测、控制/上传/启动响应验证和无动作探针任务；由于设备报告 `isEmulated=true` 且本次探针不包含运动/加热，生产物理打印完成链路仍未验收。
...
|---|---|---|---|
| 建立会话 | `GET /rr_connect?password=<password>&sessionKey=yes` | 返回 `err`；成功时可返回 `sessionKey` 和 `sessionTimeout`。后续请求使用 `X-Session-Key`。 | 已实现；Mock 验证通过；真实目标 `192.168.0.77` 空密码探测返回 HTTP 200、`err:0`、`sessionTimeout:8000` |
| 查询状态 | `GET /rr_model?key=state`、`GET /rr_model?key=job` | RRF 3.x 支持；通过 `key` 查询对象模型，响应为 `{ key, flags, result }`。 | 已实现状态、文件名、文件大小/位置和任务字段解析；真实目标 `192.168.0.77` 已返回 `state.status=idle`、空闲任务及探针任务完成信息 |
| 执行 G-code | `GET /rr_gcode?gcode=<encoded-gcode>` | 用于发送 G/M/T-code，返回 `buff`。 | 已实现并由暂停/恢复/取消/急停适配器调用；目标设备响应级验证返回 `err=0`，物理副作用未验证 |
| 上传文件 | `POST /rr_upload?name=<path>` | 请求体是原始文件内容；发送 `Content-Length`，结果包含 `err`。 | 已实现原始流上传到 `0:/gcodes`；目标设备上传探针返回 `err=0` 且目录根可见，文件列表和物理副作用未验证 |
...
|---|---|---|---|
| 暂停 | `M25` | 从其他 G-code 来源暂停 SD 卡打印；继续使用 `M24`。 | 目标设备响应级验证返回 `err=0`；真实运行任务中的暂停效果未验证 |
| 恢复 | `M24` | 开始或恢复已通过 `M23` 选择的 SD 打印文件。 | 目标设备响应级验证返回 `err=0`；Farm 上传后的任务选择和真实恢复效果未验证 |
| 取消 | `M0` 或 `M2` | `M0` 根据当前状态执行 stop/cancel 宏；`M2` 终止当前任务且当前版本行为类似 `M0`。 | 目标设备响应级验证返回 `err=0`；`cancel.g/stop.g` 副作用未验证 |
| 急停 | `M112` | 立即终止运动并关闭电机、加热器，设备需要复位或断电重启。 | 目标设备响应级验证返回 `err=0`；真实运动/加热中副作用未验证，仍按高风险动作处理 |
| 上传并开始 | `POST /rr_upload` + `M23` + `M24`，或 `M32` | 官方确认 `M32` 等价于 `M23` 后 `M24`；文件路径必须使用 RRF 路径格式。 | 目标设备探针上传及 `M32` 响应级验证返回 `err=0`；真实任务启动和路径完整性未验证 |
...
- 目标：`192.168.0.77`，访问方式：HTTP 默认端口 80，未提供设备密码。
- `GET /rr_connect?password=&sessionKey=yes`：HTTP 200，`err=0`，`isEmulated=true`，`sessionTimeout=8000`，`boardType=unknown`，响应未返回 `sessionKey`。
- 只读 `GET /rr_model?key=state`：HTTP 200，返回 `state.status=idle`，`machineMode=FFF`，并提供 `move/heat/directories` 对象入口。
- 只读 `GET /rr_model?key=job`：初始为空任务；探针任务后返回 `lastFileName=0:/gcodes/farm_t10_rrf_probe_0903.gcode`、`filePosition=196`。
- 2026-09-03 仅在真实目标 `192.168.0.77` 上验证：Farm 文件库上传无运动/无加热探针、创建任务 ID `5`、安全派发、现场确认和 `START_PRINT` 均成功；设备随后返回 `state.status=idle`。
- 同一目标的 `M25`、`M24`、`M0`、`M112` 均通过 `GET /rr_gcode` 返回 `err=0`；Farm 暂停/急停接口返回 HTTP 200；`M112` 后使用 `M999` 复位，设备恢复 `idle`。无任务时 Farm 恢复/取消返回 HTTP 422，未绕过安全校验。
- `isEmulated=true` 和 `boardType=unknown` 仍需结合具体固件构建和硬件部署确认；本次探针没有运动/加热，因此不能证明生产任务的物理副作用。此前 `192.168.0.62` 的请求属于误输入后的历史测试，不作为真实目标验收；本次 `.77` 联调未向其他 IP 发送 RRF 控制、上传或启动请求。
...
在上述事项获得更多真实设备物理证据前，已实现的控制/上传调用表示符合官方协议边界、通过 Mock 并在唯一目标设备获得响应级证据；设备特有字段、副作用和路径失败必须归类为统一的 protocol/offline/rejected 错误，不能返回假成功或调用 Moonraker。
