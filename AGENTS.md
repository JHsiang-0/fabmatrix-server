...
当前仓库只包含后端代码，没有 Vue、React 或其他前端工程。已确认实际前端仓库为 `/home/codex/workspace/farm-ui`；前端改动应在该仓库进行，后端仓库只维护契约和联调证据。
...
v3 规划注意：控制接口的 HTTP 成功不等于设备已完成动作。后续如重新设计自动派单或状态同步，必须把“命令已发送”和“设备已确认”分开建模；取消、急停或长时间 G-code 阻塞时不得提前释放打印机绑定。监控应支持空闲/打印中/离线自适应轮询、控制后的短时确认和变化去重；设备主动事件只能作为协议能力可用时的优化，不能替代心跳轮询和最终一致性校验。以上属于 v3，不得为了实现它改变 v2 的手动优先默认行为。
