...
当前已冻结消息类型和 `FarmStatusMessage` 顶层结构，并由服务端校验类型、时间戳、关联 ID 和敏感字段。鉴权成功后服务端发送一次 `SNAPSHOT`，其 `data.printers` 使用安全 `PrinterVO`，没有打印机时返回空数组；离线事件和任务事件仍属于后续 WebSocket 功能。本阶段已完成握手鉴权，生产环境不再允许匿名广播。
