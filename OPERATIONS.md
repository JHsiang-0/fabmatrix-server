...
全新 Local Edition 数据目录不预置管理员账号。首次启动后由前端调用 `GET /api/v1/auth/setup/status`，在 `setupAvailable=true` 时调用 `POST /api/v1/auth/setup/admin` 创建管理员；成功后直接返回 JWT。该接口只在 Local Edition 且数据库没有任何用户时可用，初始化后返回 409，不能再次匿名创建管理员。
