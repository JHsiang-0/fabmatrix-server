...
v3 在 v2 手动优先业务稳定后，再设计可选的后台自动派单和设备状态同步优化。核心方向包括：控制命令请求与设备最终确认分离；取消/急停超时和长指令阻塞时保留绑定并进入可恢复中间态；按设备状态自适应调整轮询频率；通过协议适配器接入可用的设备事件上报，并始终保留轮询作为心跳和一致性兜底。v3 还需要补充命令审计、幂等键、确认超时、跨进程恢复、状态延迟指标和多设备压测。上述内容不改变 v2 的手动上传、手动分配、手动确认和手动启动范围。
