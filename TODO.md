...
- [x] 已对真实目标 `192.168.0.77` 完成 RRF 空密码、只读对象模型、`M25/M24/M0/M112`、无动作探针上传和 `M32` 启动请求验证；Farm 已登记设备 ID `564`，探针任务已回到 `idle` 并完成本地任务收尾。该设备仍返回 `isEmulated=true`、`boardType=unknown`，真实运动/加热副作用不适用于本次无动作探针，仍需现场生产任务验收；`192.168.0.62` 仅保留为此前误输入的历史测试记录。
...
- [x] 已根据官方协议实现 RRF HTTP 会话、状态、G-code 控制和文件上传；真实目标 `192.168.0.77` 已完成控制、探针上传、Farm 安全任务和启动请求的响应级验证，但 `isEmulated=true`、具体运行模式和生产任务下的宏/物理副作用仍待现场验收。
...
- [x] 将 WebSocket 业务事件延迟到数据库事务提交后发布，避免回滚事务产生错误状态通知。
  - 有事务时提交后广播，回滚不广播；无事务的监控场景保持即时发布。
...
- [x] 修复打印机状态更新的成功判定。
  - Redis 锁获取成功不再直接视为数据库更新成功；只有 MySQL 实际影响记录时才允许监控任务更新本地状态并发布状态事件。
- [x] 修复恢复打印时任务/打印机状态保存和 WebSocket 事件顺序。
  - 两条状态均保存成功后才发布 `JOB_STATUS`；打印机状态保存失败不返回成功且不发布事件。
- [x] 收敛手动任务状态转换的双记录保存检查。
  - 派发、启动、取消、重新排队和安全确认不能忽略打印机或任务 `updateById` 失败；失败时回滚且不发布成功事件。
- [x] 修复设备完成/失败/取消同步时的解绑保存失败处理。
  - 打印机解绑未成功落库时，不应把任务标记为终态或广播成功事件；保留绑定状态等待下一轮监控重试。
- [x] 收敛用户创建、改密、资料更新和密码迁移的数据库写入结果。
  - 数据库影响 0 行时必须返回失败，不能让前端误以为账号或密码已经更新；登录时旧明文密码自动迁移也检查实际写入结果。
- [x] 收敛打印机新增、编辑、重新录入和删除的数据库写入结果。
  - 写入失败时不能刷新缓存或向前端返回成功。
- [x] 修复打印机编辑未传 apiKey 时误清除设备凭据的问题。
  - apiKey 不在响应中返回，编辑请求未传/空白时必须保留原值。
- [x] 收紧打印机扫描 subnet 参数。
  - 只允许三段 IPv4 网段前缀，非法输入直接返回 400，不进入局域网探测。
- [x] 收敛打印机详情、状态历史和统计的参数错误码。
  - 非正打印机 ID 或反向时间范围返回 HTTP 400/业务码400；不存在的打印机仍返回 404。
- [x] 修复状态历史写入失败后的重试语义。
  - 历史插入失败不能更新已持久化游标；不阻塞设备监控，并在后续采样时重试。
- [x] 收敛分页查询的缺失参数和任务时间范围校验。
  - 打印机、文件、用户和任务分页请求体缺失时统一返回 400；任务查询的 `startTime` 晚于 `endTime` 时返回 400，不执行数据库查询。
- [x] 收敛剩余客户端输入错误码。
  - 空文件上传、打印机录入缺失 IP、打印机更新缺失 ID、空文件夹名和缺失状态历史查询体统一返回 HTTP 400/业务码400；任务状态失败和数据库写入失败不改为参数错误。
- [x] 收敛所有任务派发路径的设备绑定冲突校验。
  - 手动安全派发和兼容派发除检查 `IDLE` 外，必须检查 `currentJobId=null`；已有绑定返回 HTTP 409，不覆盖原任务；自动调度继续跳过并等待下一轮。
- [x] 收敛任务创建的数据库插入成功判定。
  - 标准创建和 Moonraker 兼容提交必须检查 `save` 实际结果，插入失败时不得返回任务 ID或继续后续流程。
- [x] 收敛文件夹创建和单文件删除的数据库写入结果。
  - 数据库记录未成功写入/删除时返回失败，不伪装成功；对象存储残留按运维清理流程处理。
...
- [x] 修复耗材长度单位转换：G-code 毫米字段统一转换为米，不再用 `>1000` 的阈值判断，避免短文件返回错误长度。
- [x] 冻结文件和任务 VO 的数值字段类型：文件切片温度为 Integer，成功率和 REST 任务进度为 BigDecimal；实时设备状态温度仍按现有 DTO 使用 Double。
...
- [x] 文件上传数据库保存失败时补偿清理 RustFS 对象。
  - 主文件入库失败会尝试删除已上传对象；缩略图已上传时同步尝试删除；清理异常只记录日志，不覆盖原始数据库错误。
...
- [x] 修复自动调度的事务边界和双记录保存检查。
  - 自动派发已下沉到 `PrintJobService.assignQueuedJob`；调度器仅持有分布式锁并触发调用，Service 重新校验任务/打印机状态，任务或打印机保存失败会回滚且不发布成功事件。
...
- [x] ADMIN 可以创建和管理 OPERATOR；真实管理员会话已验证创建、更新、禁用、启用接口，临时验收账号已再次禁用。
- [x] ADMIN 可以添加 Klipper 或 RRF 打印机；真实目标 RRF `192.168.0.77` 已登记为设备 ID `564`，完整安全任务链路已用无动作探针完成，生产任务副作用仍待现场验收。
- [ ] ADMIN/OPERATOR 可以查看设备状态和文件库。
- [ ] 用户可以上传 G-code、创建任务、派发任务并安全启动。
- [ ] 可以暂停、恢复、取消和急停。
- [ ] 任务状态和打印机状态通过 REST + WebSocket 正确同步。
- [x] 打印机离线不会导致定时任务持续刷异常日志；监控轮次互斥且单台设备离线 WARN 日志按 60 秒限频，实际 dev 定时任务仍保持关闭。
- [ ] 文件、任务、用户和设备权限经过后端校验。
