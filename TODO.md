...
- [x] 增加异常处理器测试，覆盖核心的 401、403、404、409、422、500、503 映射；`SecurityResponseTest` 当前 25 项通过。
...
- [x] 增加连接 Token 校验。
- [x] 增加连接上限、异常断开和清理机制；连接上限由 `farm.websocket.max-connections` 配置，服务端按 30 秒可配置间隔发送协议级 Ping，失败连接自动清理，前端自动重连仍待真实前端仓库。
- [x] 统一消息结构：
...
- [x] RustFS 测试：已覆盖上传、预签名 URL、删除失败的客户端异常转换，以及文件 Service 的 URL 上限、归属和删除保护；真实 RustFS 容器联调待现场环境。
- [x] Redis 测试：已覆盖登录失败计数/锁定、用户禁用标记、打印机状态缓存 TTL 和状态锁竞争；真实 Redis 容器联调待现场环境。
- [x] WebSocket 测试：已覆盖连接鉴权、快照、状态推送、离线事件、失败任务事件和断开清理；前端自动重连待真实前端仓库。
